...
function setupEventListeners() {
    ...
    document.getElementById('diseaseForm').addEventListener('submit', handleDiseaseDetection);
    document.getElementById('diseaseImage').addEventListener('change', handleImagePreview);
    document.getElementById('resetDiseaseBtn').addEventListener('click', resetDiseaseForm);
...
async function handleDiseaseDetection(e) {
    ...
    const submitBtn = document.getElementById('diseaseSubmitBtn');
    const originalText = submitBtn.innerHTML;

    if (!fileInput.files || !fileInput.files[0]) {
        showError('Disease Detection', 'Please select an image first.');
        return;
    }
    ...
    try {
        ...
        try {
            ...
            if (result.success) {
                displayDiseaseResults(result.data);
                document.getElementById('resetDiseaseBtn').style.display = 'inline-block';
            } else {
                showError('Disease Detection', result.error || 'Failed to detect disease');
            }
        } catch (error) {
            showError('Disease Detection', 'A network error occurred. Please check your internet connection and try again.');
            console.error('Network Error:', error);
        }

    } catch (error) {
        showError('Disease Detection', 'Failed to analyze image. Please try again.');
        console.error('Error:', error);
    } finally {
        // Reset button state
        submitBtn.innerHTML = originalText;
...
function displayDiseaseResults(data) {
    ...
    const resultsBody = document.getElementById('diseaseResultsBody');
            ...
            <div class="col-md-12 mb-4">
                <div class="alert alert-info">
                    <h5 class="alert-heading">
                        <i class="fas fa-leaf me-2"></i>${data.disease_name || data.disease_class}
                    </h5>
                    <p class="mb-0">
                        <strong>Confidence:</strong> ${data.confidence ? (data.confidence * 100).toFixed(1) + '%' : 'N/A'}
                    </p>
                </div>
            </div>
        </div>
    ...
    if (data.prevention_steps) {
        html += `
            <div class="row mb-3">
                <div class="col-12">
                    <h6><i class="fas fa-shield-alt text-success me-2"></i>Prevention & Treatment</h6>
    ...
    if (data.supplement && data.supplement.name) {
        html += `
            <div class="row">
                <div class="col-12">
                    <div class="card bg-light">
                        <div class="card-body">
                            <h6><i class="fas fa-prescription-bottle text-warning me-2"></i>Recommended Supplement</h6>
                            <p class="mb-2"><strong>${data.supplement.name}</strong></p>
                            ${data.supplement.buy_link ? `
                                <a href="${data.supplement.buy_link}" target="_blank" class="btn btn-sm btn-warning">
...
function resetDiseaseForm() {
    ...
    document.getElementById('resetDiseaseBtn').style.display = 'none';
